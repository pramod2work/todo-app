const { baseUrl, maxInstance, dataSheet, tagExpression, feature } = require('../environment-properties')
...
exports.config = {
  ...
  screenshotPath: `${reportsPath}errorShots/`,
  //
  ...
  reporters: ['spec', 'json', 'allure', 'junit'],
  reporterOptions: {
    outputDir: `${reportsPath}json/`,
    allure: {
      outputDir: `${reportsPath}allure-results/`
    },
    junit: {
  ...
  cucumberOpts: {
    ...
    require: [
      `${testSourePath}src/steps/given.js`,
      `${testSourePath}src/steps/then.js`,
      `${testSourePath}src/steps/when.js`
    ],
  ...
  before: function before () {
    ...
    const workbook = XLSX.readFile(`${testSourePath}src/testdata/${dataSheet}.xlsx`)
    global.masterTestData = XLSX.utils.sheet_to_json(workbook.Sheets['Master'])
  }
  //
